...
.SYNOPSIS
  ...
  By choosing -InstallOffice2019 it tries to install Office 2019 as well.
.DESCRIPTION
  By choosing -InstallOffice2019 it tries to install Office 2019 as well.
.PARAMETER InstallOffice2019
  Will install Office2019 after removal.
...
.NOTES
  Version:        1.2
.EXAMPLE
  .\MSOffice-Remove.ps1 -SupressReboot -Force
#>

#---------------------------------------------------------[Initialisations]--------------------------------------------------------
Param (
    [switch]$InstallOffice2019 = $True,
...
$Office2019Setup_URL = "\\10.10.2.35\1809DeploymentShare\_IMAGING_\Apps\Office2019"
#-----------------------------------------------------------[Functions]------------------------------------------------------------
Function Invoke-OfficeUninstall {
    ...
    if ($UseSetupRemoval) {
        ...
        Invoke-SetupOffice2019 "$Office2019Setup_URL/purge.xml"
    }
    else {
...
Function Invoke-SetupOffice2019($Office2019ConfigFile) {
    if ($Office2019ConfigFile -eq "$Office2019Setup_URL/purge.xml") {
        Write-Host "Downloading Office2019 Installer ..."
        Start-BitsTransfer -Source "$Office2019Setup_URL/setup.exe" -Destination "$SaRA_DIR\setup.exe"
        Start-BitsTransfer -Source "$Office2019ConfigFile" -Destination "$SaRA_DIR\purge.xml"
        Write-Host "Executing Office2019 Setup ..."
    ...
    if ($InstallOffice2019) {
        Write-Host "Downloading Office2019 Installer ..."
        Start-BitsTransfer -Source "$Office2019Setup_URL/setup.exe" -Destination "$SaRA_DIR\setup.exe"
        Start-BitsTransfer -Source "$Office2019ConfigFile" -Destination "$SaRA_DIR\config.xml"
        Write-Host "Executing Office2019 Setup ..."
...
Function Set-CurrentStage($StageValue) {
    if (-not (Test-Path "HKLM:\Software\OEM\CAC-ITSA\O2019\Install")) {
        New-Item -Path "HKLM:\Software\OEM\CAC-ITSA\O2019\Install" -Force | Out-Null
    }
    New-ItemProperty -Path "HKLM:\Software\OEM\CAC-ITSA\O2019\Install" -Name "CurrentStage" -Value $StageValue -PropertyType String -Force | Out-Null
}
...
if (-not ($RunAgain)) {
    if (Test-Path "HKLM:\Software\OEM\CAC-ITSA\O2019\Install") {
        $CurrentStageValue = (Get-ItemProperty "HKLM:\Software\OEM\CAC-ITSA\O2019\Install").CurrentStage
        Switch ($CurrentStageValue) {
            1 {
                Write-Host "Resuming Stage 1: Uninstalling Office ..."
                Invoke-OfficeUninstall 
                Invoke-SetupOffice2019 "$Office2019Setup_URL/upgrade.xml"
                Remove-SaRA
                Invoke-RebootInSeconds $SecondsToReboot
            }

            2 {
                Write-Host "Resuming Stage 2: Installing Office 2019 ..."
                Invoke-SetupOffice2019 "$Office2019Setup_URL/upgrade.xml"
                Remove-SaRA
                Invoke-RebootInSeconds $SecondsToReboot
            }
