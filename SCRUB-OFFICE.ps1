...
#----------------------------------------------------------[Declarations]----------------------------------------------------------
$SaRA_URL = "https://www.dropbox.com/scl/fi/tphcrmb5tda2bkbg55k81/SaRA_EnterpriseVersionFiles.zip?rlkey=tc28jzhflsdpl8hwlca0xx2y4&st=amekyhrq&dl=1"
...
Function Set-CurrentStage($StageValue) {
    if (-not (Test-Path "HKLM:\Software\OEM\CAC-ITSA\M365\Install")) {
        New-Item -Path "HKLM:\Software\OEM\CAC-ITSA\M365\Install" -Force | Out-Null
    }
    New-ItemProperty -Path "HKLM:\Software\OEM\CAC-ITSA\M365\Install" -Name "CurrentStage" -Value $StageValue -PropertyType String -Force | Out-Null
}
...
if (-not ($RunAgain)) {
    if (Test-Path "HKLM:\Software\OEM\CAC-ITSA\M365\Install") {
        $CurrentStageValue = (Get-ItemProperty "HKLM:\Software\OEM\CAC-ITSA\M365\Install").CurrentStage
